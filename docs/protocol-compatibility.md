...
- **Classification**: the deterministic matrix classifies each exact path `EXPERIMENTAL` (a full Layer A pass — production trust requires Layer B/C evidence plus reviewed promotion, #124; `production-claim-not-established`), `BROKEN` (a required contract fails), or `unknown` (required gates unrun, never reported as passed). Since #122 removed `livePassed`/`liveEvidence`, no observation can emit `VERIFIED`; the canary never mislabels fake evidence as live proof.
...
#122 replaces the coarse `livePassed: boolean` classification input with a **versioned, feature-scoped Compatibility Claim + Evidence model keyed to the exact execution path** (`src/canary/claim.ts`):

- **Claim identity** — a stable versioned key includes client kind, exact client version/baseline, source protocol/revision (#119 `FidelitySourceProtocol` vocabulary), adapter/integration surface, access provider, auth mode, endpoint contract, exact physical model, and feature/capability claim. Model family is classification metadata only and is never part of the key: it can never replace exact provider/model/access identity. The same upstream model through two providers, and two features on the same path, always produce distinct claim keys and independent evidence histories (`claimKeyFor`, deterministic sha-256 `claimKeyHash` for file names).
- **Feature-scoped claims** — text, streaming, cancellation, tools (single/multi/parallel continuation), reasoning, reasoning+tools, model discovery, session/subagent attribution, config-overlay behavior, and other relied-on features are claimed independently; passing text never implies tool/reasoning/discovery compatibility.
- **Evidence Artifact v2** — each record carries the claim key, evidence layer/kind, fixture/corpus revision, runner version, timestamp, result (`passed`/`failed`/`not-run`; `missing` is record absence), typed failure reason, environment/platform metadata needed for interpretation, and safe references to raw machine-readable results. Evidence never contains credentials, auth headers, account identity, prompts, real responses, or reasoning text.
- **Evidence layers** — Layer A: deterministic protocol/adapter conformance with fake/synthetic fixtures (the current canary matrix is reclassified as Layer A). Layer B: exact installed-client black-box behavior. Layer C: exact real access-path live verification. Layers B/C runners are owned by #123 and are not built here; required layers are declared per adapter (`requiredLayersForAdapter`, unknown adapters fail closed A/B/C) and layer presence/result is explicit per claim, never collapsed into one boolean.
- **Removal of `livePassed` trust semantics** — `livePassed`/`liveEvidence` no longer exist on the authoritative classification path. Classification of a full deterministic pass is `EXPERIMENTAL` with `production-claim-not-established`; `VERIFIED` is unreachable from any observation (a reviewed promotion is #124). `RLY_LIVE_CANARY`/`liveRunnerEnabled` may enable a runner hook (#123) but can never stand in for an evidence artifact — enabling it emits no Layer B/C records and no VERIFIED verdict.
- **Schema/version and legacy migration** — claim documents carry `schemaVersion` 1 and evidence `evidenceSchemaVersion` 2. Pre-v2 canary artifacts are flagged legacy/untrusted (`legacy-v1-artifact-untrusted-for-v2-claims`): readable for diagnostics but never able to satisfy a v2 claim — a claim lookup returns `missing` until a real v2 observation records evidence. Registry `CompatibilityEvidence` (revision 5) carries an optional `claimRef` linking a reviewed state to its exact claim key; pre-v5 rows are untrusted for v2 claim authority.
- **Persistence/query** — claim evidence appends audit-friendly under `<control-plane>/claims/` (`ClaimEvidenceStore`): atomic writes, records appended and never silently rewritten, identical observations deduped, malformed documents fail closed, and deterministic lookup by exact claim identity + feature (`findEvidence`). `rly canary run|status` persists and reports the claim/evidence schema and legacy policy.

### Installed-client (Layer B) and live access-path (Layer C) runners (#123)

`rly canary run-b` and `rly canary run-c` execute the two evidence layers the deterministic matrix (Layer A) cannot prove — the ACTUAL installed client binary and the exact real access path — and emit feature-scoped Evidence v2 observations keyed to the exact execution path:

- **Layer B — installed-client black-box** (`src/canary/installed-runner.ts`): the runner detects and records the exact installed Claude Code / Codex CLI executable path + probe version, launches the REAL binary against a controlled local fixture server serving the client-facing wire surface RLY relies on (Anthropic Messages + `GET /v1/models` for Claude Code; OpenAI Responses + `GET /v1/models` for Codex CLI) with child-only environment isolation (`ANTHROPIC_BASE_URL`/`OPENAI_BASE_URL` → fixture, synthetic token, throwaway config dir — never real client config, never real credentials). Per-feature gates exercise request/stream framing, cancellation, tool round-trip, multi-tool continuation, parallel tools, reasoning/effort, reasoning+tools, discovery/selection, session/agent/parent attribution, tier aliases, subagent concurrency, config-overlay, and long sessions. Evidence is keyed to the EXACT observed client version (`observedVersion` in the claim identity); the reviewed supported baseline is recorded separately (`supportedBaseline`) and never implied by a black-box pass — **version drift surveillance**: a new/unknown client version is tested without becoming a supported baseline, and a changed client behavior yields a typed exact gate failure (`client-contract-drift`, `missing-agent-header`, `malformed-continuation`, `timeout-cancel-failure`, ...) keyed to that version. A missing/non-executable binary reports every gate `not-run` (`client-not-installed`), never PASS.
- **Layer C — exact access-path live runner** (`src/canary/live-runner.ts`): the runner executes ONE selected exact claim path — client + client version + source protocol/revision + adapter/integration + access provider + auth mode + endpoint contract + physical model + feature gate — through the real RLY gateway translation stack (client-facing Messages/Responses surface → direct adapter → the configured provider endpoint) using an explicit opt-in credential from the environment. Each feature gate carries its own request and its own Evidence v2 record (`text` success can never promote tools/reasoning/discovery); the same upstream model through two access providers produces distinct claim keys/evidence (no cross-provider sharing); tool continuation is proven only when the provider actually returns tool calls (otherwise `not-run`/`provider-did-not-call-tool`); policy-driven `GET /v1/models` discovery is exercised on the exact-path gateway (throwaway control-plane store with the explicit EXPERIMENTAL opt-in — the real control plane is never touched). Layer C runs ONLY with explicit opt-in (`RLY_LIVE_CANARY=1`) and available credentials; missing credentials, skipped runs, environment inability, and unexecuted gates are `not-run` (or absent) — never `passed`, never `VERIFIED`, and live runs never silently spend quota.
- **Typed failure taxonomy** (#123 scope item 9): `client-contract-drift` (incl. `missing-agent-header`, `effort-signal-lost`, `client-did-not-send-reasoning-config`, `client-did-not-send-effort-signal`), `protocol-translation-failure`, `authentication-failure`, `authentication-credentials-unavailable`, `provider-unavailable`, `provider-rate-limit`, `unsupported-feature`, `malformed-continuation`, `timeout-cancel-failure`, `environment-inability`, `client-not-installed`, `provider-did-not-call-tool`, `not-applicable-to-access-path`.
- **Artifacts and authority**: runner raw results persist secret-free (metadata only: gate, result, typed reason, timing, wire booleans) under `<control-plane>/canary-runners/` (`RunnerResultStore`, `0700`/`0600`); Evidence v2 records carry a safe `ref` to them plus additive `timingMs`. Runners are **observation-only** — they never mutate the trusted registry, tier mappings, `/v1/models` projection, or effective compatibility state; review/promotion/quarantine/enforcement is owned by #124.

### Execution layers in CI and manual runs (#123)

CI (`pnpm verify`) and manual runs separate the three evidence layers explicitly — a deterministic Layer A pass never implies live trust, an installed-client pass never promotes, and a live path only ever runs on explicit opt-in:

| Layer | Command | Runs in CI (`pnpm verify`) | Requires | Result |
| --- | --- | --- | --- | --- |
| A — deterministic fake matrix | `rly canary run` | Yes | nothing (synthetic fixtures) | Feature-scoped Evidence v2 (layer A); verdict `EXPERIMENTAL` at most, never `VERIFIED` |
| B — installed-client black-box | `rly canary run-b` | Deterministic fake-client tests only; real-binary sentinels opt-in (`RLY_CLAUDE_E2E=1` for Claude, `RLY_CODEX_E2E=1` for Codex) | Installed `claude`/`codex` binary | Evidence v2 (layer B) keyed to the exact observed version; missing binary → `not-run`/`client-not-installed` |
| C — real access path | `rly canary run-c` | Never (opt-in only) | `RLY_LIVE_CANARY=1` + credential env for the configured route | Evidence v2 (layer C); missing credential → `not-run`/`authentication-credentials-unavailable` |
