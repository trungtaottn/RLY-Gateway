...
- `.github/workflows/standalone-artifacts.yml` builds + verifies the matrix on GitHub Release publication (release tag = canonical version input) or manually, smoke-tests every SUPPORTED CI target (`rly --version` from the unpacked artifact), uploads tarballs + sha256 + manifest, and attaches them to the GitHub Release. Release signing/SBOM (#128) and installer/updater UX (#129) remain out of scope.
