...
### 6.15 Verified installer, updater, and uninstaller (#129)

RLY owns the end-user acquisition lifecycle on top of the #35/#128 artifact lineage and the #94 stable bootstrap. **INSTALL != ACTIVATE is the hard boundary**: acquisition may download/verify/install a candidate, but ONLY Wave 4 (#92/#93) may switch the serving `active` reference or restart the resident service.

- **Verified remote acquisition** (`src/installer/`, consumed by `rly install` and `rly update --channel`): resolve the selected channel/platform/arch through the #128 signed channel metadata (`rly-channel-<channel>.json` + `.sig`) and signed canonical release manifest (`rly-release.json` + `.sig`) — Ed25519 verified against the committed release public key (embedded in the runtime; the private key never leaves `RLY_RELEASE_SIGNING_KEY`) — then download the EXACT immutable artifact from GitHub Releases/approved origin and verify, BEFORE any install/staging mutation: channel rollback (monotonic version vs the durable highest observed), staleness (`updatedAt` + `staleness.maxAgeDays`), freeze (explicit marker), exact platform target + channel qualification gate (stable requires `supported` + `qualified`; beta permits documented `experimental-gaps` but never `not-qualified`), artifact sha256/size, the Ed25519 digest statement (`<tarball>.sig` over `sha256:<hex>`), and the unpacked-tree content-addressed digest equals the manifest `artifactDigest` with `rly-build.json`/`rly.json`/`rly-artifact.json` identity consistency. Wrong platform, stale/invalid metadata, digest mismatch, missing/invalid signature, and unknown release identity all FAIL before mutation with actionable secret-free errors.
- **Bootstrap installer** (`scripts/install.sh`): a minimal, audited, fail-closed POSIX-sh bootstrap for clean per-user macOS/Linux installs with no source checkout/npm/pnpm/user Node and no sudo. It downloads the signed metadata + exact artifact into a private staging dir, verifies the artifact digest chain in-shell (sha256 against the published `.sha256` and against the digest embedded in both signed metadata documents, plus the Ed25519 digest statement via OpenSSL 3 `pkeyutl -rawin`; a working OpenSSL is auto-detected via PATH/Homebrew kegs/`RLY_OPENSSL_BIN` and fails closed when absent), unpacks, and runs `rly install --artifact <tarball> --metadata-dir <staging>` which re-verifies the FULL canonical-JSON signature chain before any install mutation — unverified content is never executed/installed.
- **Stable local ownership/layout**: installs the #94 RLY-owned bootstrap (`<control-plane>/bootstrap/rly-gateway`) as the ONLY service execution identity (never `dist/cli/init.js`, never an incidental Node path, never a direct `runtime/refs/...` path), materializes the initial committed `active` deployment from the #35 canonical artifact tree, and uses deterministic per-platform private/user-safe locations: `~/.rly` control plane, `~/.rly/bootstrap/rly-gateway`, and a user launcher symlink `~/.local/bin/rly` → bootstrap (no sudo). Foreign/unowned paths are never overwritten (the launcher symlink fails closed on a foreign occupant).
- **First install vs update boundary**: first install runs/guides `rly init` (per-user service registration + initial committed deployment) and then `rly config`; update acquisition installs/verifies a candidate and returns a `VerifiedCandidate` to Wave 4 without changing the #92 `active` reference or restarting the resident service. `--install-only` stops after staging for the local candidate path too; a later plain `rly update` resumes activation.
- **Updater/channel policy**: `rly update` selects `beta`/`stable`/`current` channel policy (explicit switches are auditable in `<control-plane>/installer/acquisition-log.json`; `current` keeps the installed build identity's channel, `dev`/unknown → `stable`), inspects the current exact build identity, resolves a newer qualified artifact, and reports current exact build, candidate exact build/channel, verification state, and pending/activation status. Downgrade/rollback respects signed metadata (monotonic channel version) and Wave 4 state compatibility.
- **Uninstall lifecycle**: `rly uninstall` stops/disables/removes ONLY RLY-owned launchd/systemd definitions, removes ONLY RLY-owned product artifacts (`bootstrap/`, `runtime/`, `installer/`, installation + update state), and preserves `~/.rly` config/accounts/credential state by default. `rly uninstall --purge --yes` is an explicit destructive removal of the whole RLY control plane that requires unambiguous intent (`--purge` alone refuses) and never removes unrelated native Claude/Codex configuration.
- **Repair/reinstall**: re-running `rly install`/`rly init` verifies ownership/build identity and repairs missing/stale bootstrap + service definitions idempotently (a valid committed deployment is never rewritten); a differing verified version is reported as an update handoff, never activated.
- **Platform behavior**: the matrix matches #35/#128; unsupported OS/arch/service-manager environments fail explicitly with actionable messages, never partial success.
- **Privacy/security**: installer/update logs and durable records carry channel/version/build/digest/platform/path/status metadata only — never credentials, tokens, auth headers, email/raw account identity, prompts, responses, or reasoning text; downloads are private (`0600`/`0700`) and nothing is committed to git.
- **Qualification integration**: the #128 exact-byte qualification matrix includes a `verified-install` gate exercising the installer/updater verification machinery on the exact artifact bytes (positive verified + tampered-refused); real release signatures are verified post-publish by `verify-release.mjs`.
