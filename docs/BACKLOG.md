...
| BL-042 | [#23](https://github.com/trungtaottn/RLY-Gateway/issues/23) | landed (#23) | Frozen reviewed registry with a discovery→proposal boundary (#67): `proposeRegistryChanges()` diffs catalog snapshots and returns proposed candidates without mutating trusted evidence; propose-only catalog refresh pipeline and `rly admin models refresh|proposals` with deterministic drift reports and metadata-only artifact persistence separate from trusted evidence | Reviewed promotion of proposed evidence to the trusted registry (still outstanding; #72 landed the gateway projection surface that consumes only trusted evidence, #69 landed the provider/family tier resolver and a frozen reviewed tier mapping); #24 canary evidence attachment. Refresh remains propose-only and never mutates trusted registry evidence, profile tier mappings, or `/v1/models` projection |
| BL-043 | [#24](https://github.com/trungtaottn/RLY-Gateway/issues/24) | landed (#24) | Runtime compatibility evidence gate: exact installed client version reporting (`src/targets/versions.ts`), pinned baseline wire-contract fixtures, deterministic fake gate matrix, per-access-path VERIFIED/EXPERIMENTAL/BROKEN/unknown classification, secret-free evidence artifacts under `<control-plane>/canary/`, `rly canary run|status`, doctor version/baseline reporting, and #72 projection gating (VERIFIED default, EXPERIMENTAL opt-in, BROKEN never) | Reviewed promotion of canary-proposed compatibility state to the trusted registry remains an explicit control-plane action (#23/#67 boundary); live provider evidence stays opt-in (`RLY_LIVE_CANARY=1`, skipped ≠ pass) |
...
- Standalone runtime artifact distribution (#35, landed): the PRIMARY distribution is the RLY-owned standalone artifact built by `scripts/standalone/` (bundled pinned Node, compiled runtime, bundled prod deps, licenses/notices, `rly.json`/`rly-build.json`/`rly-artifact.json`) built on GitHub Release publication and verified/smoke-tested before release, then published through the SIGNED release supply chain (#128, landed): canonical `rly-release.json` manifest, per-artifact SBOMs + provenance referencing the exact digest, Ed25519 signing + signed `rly-channel-<channel>.json` metadata (rollback/staleness/freeze), exact-byte qualification as the publication authority, and release immutability enforcement. Distribution is deliberately split from activation: #73 (landed) owns the drain/restart/verify/rollback lifecycle once a candidate is obtained/verified, via an injectable `CandidateInstaller` contract (local candidate swap + fake-runner tests); #129 (landed) owns the verified installer/updater/uninstaller UX on top of the canonical artifact lineage: `rly install` / `rly update --channel beta|stable|current` / `rly uninstall` / `scripts/install.sh` with signed-metadata verification (INSTALL != ACTIVATE; Wave 4 owns activation).
