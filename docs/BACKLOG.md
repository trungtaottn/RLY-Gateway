...
- Standalone runtime artifact distribution (#35, landed): the PRIMARY distribution is the RLY-owned standalone artifact built by `scripts/standalone/` (bundled pinned Node, compiled runtime, bundled prod deps, licenses/notices, `rly.json`/`rly-build.json`/`rly-artifact.json`) built on GitHub Release publication and verified/smoke-tested before release. Distribution is deliberately split from activation: #73 (landed) owns the drain/restart/verify/rollback lifecycle once a candidate is obtained/verified, via an injectable `CandidateInstaller` contract (local candidate swap + fake-runner tests); #128 (BACKLOG) owns release signing/notarization/SBOM/provenance publication and #129 (BACKLOG) owns the verified installer/updater UX on top of the canonical artifact lineage. Until those land, `rly update --candidate <dir> [--version <v>]` installs a locally verified candidate through the existing install layout and the identity/version handshake.
