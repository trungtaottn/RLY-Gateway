...
| BL-042 | [#23](https://github.com/trungtaottn/RLY-Gateway/issues/23) | landed (#23) | Frozen reviewed registry with a discovery→proposal boundary (#67): `proposeRegistryChanges()` diffs catalog snapshots and returns proposed candidates without mutating trusted evidence; propose-only catalog refresh pipeline and `rly admin models refresh|proposals` with deterministic drift reports and metadata-only artifact persistence separate from trusted evidence | Reviewed promotion of proposed evidence to the trusted registry (owned by #69/#72); #24 canary evidence attachment |
