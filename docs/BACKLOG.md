...
| BL-042 | [#23](https://github.com/trungtaottn/RLY-Gateway/issues/23) | partial | Frozen reviewed registry with a discovery→proposal boundary (#67): `proposeRegistryChanges()` diffs catalog snapshots and returns proposed candidates without mutating trusted evidence | Propose-only catalog refresh command |
