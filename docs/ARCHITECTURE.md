...
- **Drain → restart → verify → rollback.** `rly update` installs the candidate, records pending activation, waits for the launch-session count to reach zero (unless the explicit `--force` destructive path), transitions the old runtime to draining via authenticated `POST /drain` (refuses new issuance), re-registers the service definition and restarts through the per-user service manager, verifies the new runtime (attested identity + authenticated `/readyz` + state/schema compatibility + serving version matches the pending candidate), then marks `active`. Any failure triggers exactly one bounded rollback to the preserved previous known-good version; if both fail, the state becomes `failed` with a deterministic doctor action. Foreign/unattested port owners fail closed and are never signaled.
- **Concurrency and crash recovery.** An ownership-aware update lock (pid + process start identity, stale-lock reclaim like the startup lock) serializes concurrent `rly update` invocations. Crash/reboot states recover deterministically: interrupted install → `failed` with retry guidance; pending activation → resume once the candidate verifies; interrupted activation → rollback using the preserved previous-version reference.
