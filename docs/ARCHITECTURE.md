...
- **CI/distribution**: `.github/workflows/standalone-artifacts.yml` builds + verifies the matrix on GitHub Release publication (the release tag is the canonical version input) or manually (`workflow_dispatch`), smoke-tests every SUPPORTED target from the unpacked artifact, uploads tarballs + sha256 + `artifacts.json` manifest, and attaches them to the GitHub Release. Release signing/SBOM (#128) and installer/updater UX (#129) remain out of scope; `rly-artifact.json` is the SBOM input.
