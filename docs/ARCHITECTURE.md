...
The installed execution identity is the #94 service/identity boundary on top of the #92 immutable store and #93 transactional activation. Service managers (launchd/systemd) reference ONE stable RLY-owned launcher, and activation/rollback verify the exact bytes now serving.

- **Stable bootstrap contract.** `rly init` installs `src/runtime/bootstrap.ts::writeBootstrapScript` → `<control-plane>/bootstrap/rly-gateway` (`0700` dir, `0755` owner-executable POSIX sh). The registered plist/unit ExecStart is `[bootstrap, gateway, start, --config, <abs-config>]` — it NEVER references `dist/cli/init.js` (a module, not an entrypoint), a direct `runtime/refs/...` deployment path, or the Node binary that happened to invoke `rly init`. The launcher is self-locating, resolves ONLY the committed `refs/active` deployment (refuses staged/uncommitted/missing candidates with an actionable message and exit 78), validates the immutable deployment (real current-user `0700` directory, matching `.rly-deployment.json` artifact identity, real `dist/cli/main.js` dispatcher), resolves Node (RLY_NODE → PATH → recorded re-validated fallback; never part of the definition), exports `RLY_SERVING_ARTIFACT`, and execs the real dispatcher. `resolveActiveDeployment` is the shared TS surface used by init/doctor/status/tests.
- **Initial committed deployment.** `rly init` establishes the initial `active` deployment from the installed runtime tree (`ensureInitialActiveDeployment`): allowlisted `package.json` + `dist` + `node_modules` + `LICENSE` + `rly.json` materialized PRESERVING pnpm's relative in-tree symlink layout (dereferencing `node_modules/.pnpm` sibling links breaks transitive resolution, e.g. fastify→avvio; unsafe absolute/escaping links are refused, pnpm metadata/.bin/test artifacts pruned, and the #92 artifact digest stays deterministic by hashing relative links as `link:<target>`), then installed through the #92 store primitives (`setActiveReferences`). Re-init/doctor never re-copies or rewrites a valid committed deployment (INSTALL != ACTIVATE and immutable-store rules preserved); `rly update` also keeps the definition on the bootstrap, so the controlled restart after the `active` ref switch automatically boots the newly committed deployment.
- **Exact build identity.** `src/runtime/build-identity.ts` defines ONE versioned identity (`identitySchemaVersion` 1): semantic version, commit/source revision, build ID, release channel (`dev`/`beta`/`stable`), control/data protocol versions, durable state/schema version, and the serving artifact digest (#92 content-addressed deployment identity when serving from the store). `scripts/write-build-identity.mjs` generates `dist/rly-build.json` inside the built tree at build time (`pnpm build`); `currentBuildIdentity()` merges it with `RLY_SERVING_ARTIFACT` exported by the bootstrap. The SAME fields are compared by `/identity.build`, `rly --version`, doctor/status (`update.buildIdentity`, `bootstrap`), the release-candidate manifest (`rly.json` optional `buildId`/`commitRevision`/`releaseChannel`/protocol versions), deployment metadata (optional `buildId`/`commitRevision`/`releaseChannel`), and update probation. `exactIdentityMatch` requires every field equal; `sameSemanticVersionDifferentArtifact` distinguishes two artifacts sharing a semantic version.
- **Attestation integration (no parallel identity service).** The ownership record's executable fingerprint is now `buildIdentityDigest(serving identity)` (including the artifact digest); `reusableRecord` requires the serving identity digest to match the record, so a same-semantic-version-different-artifact record/process pair fails closed as `attested-incompatible` and foreign/unattested listeners keep failing closed and are never signaled. `/identity` carries `build`; `inspectRuntimeGateway` surfaces it on `attested-compatible`. Update probation (`verifyCandidateRuntime`) requires, whenever the serving runtime identifies its artifact, that the exact artifact digest equals the candidate's immutable deployment identity + matching semantic version and control protocol — a same-semantic-version-different-artifact candidate fails probation and rolls back once; a runtime that does not identify an artifact (legacy/dev) falls back to the #73 version/protocol/readiness gate.
- **Service-definition reconciliation.** `src/service-manager/reconcile.ts` detects missing/stale/path-drifted definitions by comparing the on-disk definition with the adapter's rendered expected content (`renderDefinition`, added to both adapters) and repairs idempotently through the existing `register()` (atomic rewrite + change-only reload). `rly init` reconciles before starting; `rly doctor` detects AND repairs (reporting `repaired`); `rly status` reports read-only. Legacy definitions (Node + `dist/cli/init.js`, direct `runtime/refs/...` paths) are classified `migrated` and rewritten to the bootstrap contract without provider/account reconfiguration; no-root per-user semantics are preserved and unsupported platforms stay explicit. A service definition can never intentionally target a staged or deleted deployment because it references only the stable bootstrap.
- **Diagnostics/privacy.** `rly status`/`rly doctor`/`rly gateway status` expose bootstrap path + install state, committed active deployment identity, expected (active-deployment metadata) vs serving (attested `/identity`) build identity + match, the same-semantic-version-different-artifact signal, and service-definition revision + reconciliation state — identifiers and paths only; no credential, auth, session, or user-content data ever enters service definitions, the bootstrap script, deployment metadata, or build identity.

### Standalone runtime artifacts (#35) — primary production distribution

Standalone RLY-owned runtime artifacts are the PRIMARY production distribution: a self-contained package per supported target that installs and runs WITHOUT a user-provisioned Node/npm/pnpm toolchain or a source checkout. GitHub Releases is the artifact origin; npm/Homebrew (if introduced) are secondary convenience channels that must consume the SAME canonical artifact lineage and never build semantically different runtime bytes under the same RLY build identity. External Node is a development concern only.

- **Artifact contract** (`scripts/standalone/`): the builder consumes a clean compiled tree (`pnpm build`) + the frozen lockfile + the pinned Node version (`scripts/standalone/node-version.json`) and produces a deterministic layout per target — `rly` (self-locating POSIX-sh launcher that execs the BUNDLED `bin/node` against `dist/cli/main.js`), `bin/node` (+ `node.LICENSE`), `dist/` (compiled runtime incl. the #94 `dist/rly-build.json`), `node_modules/` (prod install; the pnpm virtual-store layout with relative in-tree symlinks is PRESERVED — dereferencing breaks transitive resolution; pnpm metadata, `.bin` shims that embed absolute store paths, and dependency test artifacts are pruned), `package.json`, `LICENSE`, `docs/third-party-notices.md`, `rly.json` (the #73/#93 candidate manifest), `rly-build.json` (exact #94 build identity, byte-identical to `dist/rly-build.json`), and `rly-artifact.json` (artifact schema, target platform/status, bundled Node version + source, content-addressed artifact digest, file count, source date epoch, allowlist version). Normal execution never depends on the source repo, the pnpm workspace, a user Node, or the invoking CWD.
- **Platform matrix**: every target (`darwin-arm64`, `darwin-x64`, `linux-x64`, `linux-arm64`) is EXPLICIT — `supported` (built AND smoke-tested on a qualified runner; today `linux-x64` on the repository CI) or `experimental` (built deterministically; smoke-testing requires an unprovisioned runner — the other three) — with a stated reason. Each target maps to its own Node distribution tarball; a target never silently reuses another target's artifact or Node bytes.
- **Positive allowlist + absence gates**: only compiled runtime code, bundled runtime deps, required assets/templates, licenses/notices, and explicit bootstrap resources ship. Packaging and verification (`verifyArtifactDirectory`) enforce the exact top-level allowlist, forbidden path markers (`.git`, `.env`, `~/.rly`, `.agent-gateway`, `plans/`, `tests/`, `coverage/`, `reports/`, snapshots, sqlite, logs, keys/certs, pnpm metadata), symlink safety (relative, in-tree only — escaping/absolute links refused), and a secret-content scan over RLY-owned files (same rules as the privacy scan). Any violation FAILS the build.
- **Canonical identity**: `scripts/write-build-identity.mjs` consumes the canonical version authority (`RLY_RELEASE_VERSION` → exact git tag → `package.json`), resolving the `0.1.0`-vs-release-tag ambiguity at the source. The artifact emits the #94 fields consistently across `rly-build.json`/`rly.json`/`rly-artifact.json`; a diverging identity version, a `dist/rly-build.json` mismatch, or a digest mismatch fails the build. The artifact digest is content-addressed over the exact tree (sorted relative paths + file digests + relative symlink targets — the same rules as the #92 immutable store), so two byte-distinct artifacts cannot claim the same identity and the runtime's #94 exact-identity comparison distinguishes them.
- **Reproducible build**: deterministic sorted file lists, fixed `SOURCE_DATE_EPOCH` mtimes, uid/gid 0, no owner names, no absolute developer paths/tokens/home dirs or pnpm store paths embedded; identical inputs produce byte-identical tarballs and digests (verified by hermetic tests and repeat builds).
- **CI/distribution**: `.github/workflows/standalone-artifacts.yml` builds + verifies the matrix on GitHub Release publication (the release tag is the canonical version input) or manually (`workflow_dispatch`), smoke-tests every SUPPORTED target from the unpacked artifact, QUALIFIES the exact artifact bytes and publishes the signed release supply chain (see Release supply chain below), uploads tarballs + sha256 + `artifacts.json` + signing/SBOM/provenance/channel assets, and attaches them to the GitHub Release. Installer/updater UX (#129) remains out of scope; `rly-artifact.json` is the SBOM input.
- **Consumption**: the artifact root is a valid runtime package root for the #94 initial-deployment materialization (`package.json + dist + node_modules + LICENSE + rly.json`), and the deterministic layout is the contract the #129 installer will install and the #94 bootstrap will serve.

### Release supply chain (#128) — authenticity, evidence, exact-byte qualification

RLY publishes the §6.13 artifact lineage through a signed release supply chain (`scripts/release/`). **Exact-byte qualification is the publication authority**: a release is promoted only on evidence produced by installing and exercising the EXACT artifact digest that is subsequently published — never a rebuilt equivalent, never a version label.

- **Metadata is separated from artifacts (TUF-style).** The canonical release manifest (`rly-release.json`), per-artifact SBOMs (`rly-<target>.sbom.json`), provenance (`rly-provenance.json`), qualification (`rly-qualification.json`), and signed channel metadata (`rly-channel-<channel>.json`) are SIBLING release assets beside the tarballs — evidence is attached/referenced to the exact artifact digest and is NEVER embedded in the artifact (embedding would change the digest it describes). The updater must verify the signed metadata; it never trusts a mutable GitHub `latest` redirect alone.
- **Canonical release manifest** (`scripts/release/manifest.mjs`) binds product version, channel, source commit, build ID, every supported target, artifact filename/size/sha256/content-addressed digest, bundled runtime version, state/protocol compatibility, required signatures/attestations, and workflow/toolchain inputs — consistent with the #94 exact build identity (`releaseManifestMatchesIdentity`; a split identity fails publish/verify).
- **SBOM + provenance** (`scripts/release/sbom.mjs`, `scripts/release/provenance.mjs`): an SPDX-2.3-style SBOM is generated from the ACTUAL packaged bytes (RLY components, bundled pinned Node, third-party `node_modules` packages; deterministic sorted components, stable namespace derived from the artifact identity) and references the exact digest via `artifactRef`; provenance ties every artifact digest to the exact source revision + release workflow/toolchain inputs (SLSA-flavored, secret-free).
- **Platform authenticity.** macOS production artifacts must pass the documented code-signing/notarization/stapling verification gate (`codesign --verify --deep --strict` + `stapler validate` on a provisioned macOS host with Apple certificate secrets) before stable promotion; Linux authenticity uses the per-artifact Ed25519 signature (`<tarball>.sig` over `sha256:<hex>`) + the release manifest trust chain. A missing required platform signature BLOCKS promotion for that target (machine-readable). Signing keys are Ed25519; the PRIVATE key is only the repository secret `RLY_RELEASE_SIGNING_KEY`, the PUBLIC key is committed (`scripts/release/signing-public-key.pem`, `scripts/release/signing.mjs`).
- **Signed channel metadata** (`scripts/release/channel.mjs`) maps beta/stable to exact release/build/artifact digests with explicit semantics: rollback protection via a monotonic per-channel `version` counter (lower-than-observed is refused), staleness via `updatedAt` + `staleness.maxAgeDays` (stale is refused), and an explicit `freeze` marker blocking activation beyond the frozen snapshot.
- **Exact-byte qualification** (`scripts/release/qualification.mjs`, `scripts/release/qualify.mjs`): the matrix covers clean install, `rly --version`/build identity, permissions, platform signing, runtime readiness (doctor ok), update handoff contract (the #73/#92/#94 candidate contract), `rly init`/service registration, and uninstall (self-containment). Gates are host-aware (a target the runner cannot execute is recorded `skipped`, never passed); a skipped/failed gate prevents stable qualification (`qualificationBlocksStable`).
- **Release immutability** (`scripts/release/immutability.mjs`): published bytes/digests are never silently replaced under the same release identity — republishing the same version with different digests fails (`assertReleaseImmutable`) and byte replacement of published assets is detected against the signed metadata (`detectAssetReplacement`).
- **Publisher/verifier CLIs** (`scripts/release/publish.mjs`, `scripts/release/verify-release.mjs`): the publisher consumes the #35 `out/standalone` lineage and emits the signed assets (canonical manifest, per-artifact SBOMs + signatures, provenance, channel metadata, qualification reference); the verifier validates shape, identity consistency, exact-digest binding of every SBOM/provenance subject, Ed25519 signatures, channel evaluation (rollback/staleness/freeze), beta vs stable qualification gates, and immutability — exit non-zero on any failure.
- **Workflow hardening + privacy**: release-critical third-party Actions are pinned to reviewed immutable commit SHAs and release workflows use least-required permissions with no npm credentials (`scripts/check-release-supply-chain.mjs` is part of the release gate). All supply-chain assets contain public build metadata only — never credentials, tokens, local state, private source-history content, prompts, responses, or account identity.

### Verified installer/updater/uninstaller (#129) — acquisition lifecycle

RLY owns the end-user acquisition lifecycle on top of the #35/#128 lineage and the #94 bootstrap. **INSTALL != ACTIVATE**: acquisition may download/verify/install a candidate, but ONLY Wave 4 (#92/#93) switches the serving `active` reference or restarts the resident service.

- **Acquisition module** (`src/installer/`): `signing.ts` (Ed25519 verification + canonical JSON, mirroring the publisher `scripts/release/signing.mjs`; the release PUBLIC key is embedded in `release-key.ts` and cross-tested byte-identical to the committed PEM), `metadata.ts` (signed channel metadata + release manifest resolution, rollback/staleness/freeze evaluation, target/qualification gates, release listing + asset URL derivation), `acquire.ts` (orchestration: download → sha256/size → digest statement → unpack → unpacked-tree digest + identity consistency → `VerifiedCandidate`; `verifyLocalAcquisition` re-verifies an already-downloaded chain — the bootstrap-installer handoff), and `state.ts` (durable secret-free `<control-plane>/installer/`: `observed-channels.json` for rollback protection + bounded `acquisition-log.json` audit).
- **CLI surface**: `rly install` (first install via the `rly init` contract with `packageRoot` = the verified artifact tree; repair/reinstall verifies ownership/build identity and repairs bootstrap + service definitions idempotently; a differing verified version is an update handoff, never activated), `rly update --channel beta|stable|current` (acquisition → stage via the #73/#93 lifecycle's `stageCandidate`/`installOnly` → `VerifiedCandidate` handoff; a later plain `rly update` resumes activation), and `rly uninstall` (removes ONLY RLY-owned definitions + product artifacts, preserves `~/.rly` durable data; `--purge --yes` explicit destructive removal).
- **Bootstrap installer** (`scripts/install.sh`): fail-closed POSIX sh for clean per-user macOS/Linux installs — no source/npm/pnpm/user Node, no sudo. Verifies the artifact digest chain in-shell (sha256 vs published `.sha256` AND both signed metadata documents; Ed25519 digest statement via OpenSSL 3 `pkeyutl -rawin` against the committed public key; a working OpenSSL is auto-detected via PATH/Homebrew kegs/`RLY_OPENSSL_BIN` and fails closed when absent), then runs `rly install --artifact <tarball> --metadata-dir <staging>` which re-verifies the FULL canonical-JSON signature chain before any install mutation.
- **Deterministic install locations** (per-platform, private/user-safe, no sudo): control plane `~/.rly`, stable bootstrap `~/.rly/bootstrap/rly-gateway`, user launcher symlink `~/.local/bin/rly` → bootstrap; a foreign/unowned occupant of the launcher path is refused, never overwritten.
- **Qualification integration**: the #128 matrix includes a `verified-install` gate exercising the acquisition machinery on the exact artifact bytes (positive verified + tampered-refused); real release signatures are verified post-publish by `verify-release.mjs`.
...
### Incremental streaming transport (#120)

Wave 1 streaming transport execution (Anthropic Messages and OpenAI Responses) is incremental and long-session-safe:

- **Incremental encoders** (`src/protocols/anthropic/encoder.ts`, `src/protocols/openai-responses/encoder.ts`): per-stream encoder state consumes ONE canonical event at a time and returns only the new wire frames it produces. The Anthropic encoder retains only the monotonic sequence/provenance guard plus the open-block map (block index → content type, needed for `signature_delta` fail-closed ordering); the Responses encoder additionally retains the aggregate output projection (items + per-block text/reasoning/tool-JSON accumulators + open set) required to emit the terminal `response.completed` payload and to persist continuation. Neither retains the canonical event history, so N events cost O(N) work with O(open blocks / output size) retained state.
- **Single-pass pump** (`src/routes/stream-pump.ts`): both routes stream through one shared pump that pulls an upstream event, feeds the incremental encoder, and yields ONLY the new frames — never a re-encode of prior output. `finish()` validates terminal protocol state (no open content) on a clean stream end, matching the batch `final = true` encoder.
- **Backpressure**: the pump suspends at `yield`, so a backed-up downstream readable stops pulling, which pauses upstream iteration; buffering stays bounded by the readable high-water mark. The idle/progress clock is disarmed while suspended on downstream backpressure, so a slow consumer never causes a spurious timeout; frames are never lost, duplicated, or reordered.
- **Cancellation/disconnect**: client abort/disconnect promptly aborts upstream work through the merged AbortSignal (client disconnect + policy timeouts); no frames are emitted after cancellation/close, and an upstream that ignores abort is stopped at the next pump terminal check.
- **Stream timeout policy** (`src/routes/stream-lifecycle.ts`): a connection/setup timeout (first frame within `setupTimeoutMs`, default 30 s) is separate from the healthy stream lifetime; after the first frame an idle/progress timeout (`idleTimeoutMs`, default 5 min) measures gaps between upstream events while the downstream is writable. There is NO generic whole-request timer, so healthy long-lived agent streams with ongoing deltas are never killed; a timeout emits exactly one in-band structured error frame while the client is still connected. Routes accept an optional `streamTimeouts` override; defaults apply otherwise.
- **Exactly-once cleanup**: iterator (`return()`), encoder `finish()`, timers, client-abort listeners, and the response are cleaned exactly once on completion, error, timeout, disconnect, and teardown. The client abort binding is released by the pump's `onFinished` (the route handler returns before the stream ends, so binding until stream end is required for prompt mid-stream cancellation). Terminal classification distinguishes clean completion from error, cancellation, and setup/idle timeout, and a clean end overrides a spurious close-after-end cancellation.
- **Observability/privacy**: stream metrics are metadata only — event count, frame count, backpressure count, duration, terminal kind (completed/error/cancelled/timeout with setup/idle category). Prompts, responses, reasoning text, tool payloads, signatures, and credentials are never recorded.
...
Model selection is a deterministic stage that runs **before** account selection (#68). Given an access provider, an optional preferred model family, an optional exact physical model pin, the decoded request's required capabilities, and a reasoning intent, the capability matching engine (`src/routing/model-selection/`) retrieves candidates from the trusted model intelligence registry, applies hard eligibility (exact evidence, protocol capabilities via the existing `CapabilityRequirement` semantics, reasoning semantics from `ReasoningCapabilityEvidence`, and compatibility state), then ranks deterministically in reviewed registry document order. It returns one frozen physical `ModelEvidence` target plus a secret-free decision trace; a typed failure (`unknown-exact-model`, `no-trusted-evidence`, `capability-unsupported`, `reasoning-unsupported`, `reasoning-translation-unsupported`, `reasoning-budget-policy-missing`, `compatibility-rejected`, `no-eligible-candidate`) is raised when no candidate is eligible. **The FINAL model-control output before account selection is the typed `EffectiveModelDecision` (#127)**: it assembles the selector intent (#125), the frozen target + provenance (#68/#69/#71/#72), the reasoning mapping (#70), the ECR compatibility reference (#124), the provider→pool binding, revisions, view/env-settings ownership, visible conflicts, blocked alternatives, and stable reasons into ONE explainable object (see the EffectiveModelDecision control plane section below). The existing `RouteSelector` still owns account eligibility, affinity, quota, pin order, retry safety, and credential generation within one pool; it never selects between physical models, and account retry/failover cannot change the frozen model. `BROKEN` compatibility is always rejected; `EXPERIMENTAL` is rejected by the default normal-user policy on the candidate path and requires an explicit opt-in (an explicit exact-model pin is itself an opt-in for that exact model). #69 owns tier/family preference ranking and cross-family fallback policy.
...
RLY assembles every model-routing input into ONE typed, secret-free `EffectiveModelDecision` (`src/routing/model-decision/`) — the **FINAL model-control output produced BEFORE account selection**. It is a recorder, not a second authority: it consumes the existing #68 selection trace, #69 tier trace, #70 reasoning result, #71 parent context, #72 projection/pool binding, #124 ECR answers, #125 intent classification, and #126 view/env-settings ownership state, and it never re-resolves a model, tier, projection, or compatibility answer and never touches accounts/credentials. It does not create a second model registry, tier resolver, account selector, or compatibility authority.

- **Decision contract**: one `EffectiveModelDecision` per supported routing request (profile, projected, subagent, Codex), produced in `resolve-route.ts` before the pool/account `RouteSelector` runs and carried on `ProfileDecisionTrace.effectiveModelDecision` plus the returned route. It records the #125 selector intent + provenance, deterministic precedence bookkeeping (winner, full documented order, resolution path), visible conflicts, the FROZEN physical access-provider/model/family/logicalId/adapter target, target provenance (#72 projection reverse mapping, #69 tier mapping, #125 client-alias mapping, #71 parent context, #68 exact-model, profile role, #126 persisted-view state, env ownership), the #70 reasoning intent→native mapping, the #124 ECR compatibility reference (`authority`/label/enforcement/seed/per-feature answers), the #72 provider→pool binding + session-universe revision, policy/profile/session/registry/mapping revisions, stable safe reasons, and blocked alternatives with typed failure reasons. Account/credential identity is NEVER part of the object.
- **Deterministic precedence** (highest → lowest): explicit exact selection → RLY logical tier → client-native alias/model → subagent inherit/override → profile/session policy → env/settings ownership → persisted RLY-view state → defaults. The request's explicit selection always wins; conflicts (persisted-view model vs request, launch-policy pin vs request, subagent explicit selection vs frozen parent context, foreign/stale projection vs owning view, gateway-contract env keys present) are visible and deterministic — no hidden string/env override silently wins.
- **Freeze-before-account**: the physical provider/model target + reasoning policy are frozen in the decision BEFORE the existing pool/account `RouteSelector`; account retry/failover inside the pool never changes the physical model or selector meaning (proven across a two-account rotation).
- **Subagent/parent affinity**: child intent/reasoning stays isolated; the parent/main session model, profile mapping, and execution context are never mutated by a subagent decision (#71 retained).
- **Persisted/resume policy**: a decision reads persisted RLY-owned `claude-rly-*` state ONLY from its owning profile view (`readPersistedViewModel`, #126 derived view id) and only as routing metadata; a stale/foreign projection resolves only through the session's pinned universe reverse mapping and fails closed (`model-unavailable`) when absent — another profile's persisted model never silently becomes the effective target.
- **Codex boundary**: Codex model/provider control uses the same intent/decision vocabulary without forcing Claude-specific aliases/config semantics onto Codex; client-specific behavior stays gated by exact #124 compatibility evidence.
- **Diagnostics/privacy**: `rly route-trace` carries the secret-free decision; `rly doctor` reports the model-control contract (schema version, precedence order, ECR final authority). Output is allowlisted metadata only — never prompts, responses, reasoning text, credentials, auth headers, raw account identity, or full user settings content.
...
- **Session freeze for profile routes (#J2 contract A)**: session pinning covers BOTH the projection namespace AND the profile-route execution target. At `POST /v1/launch-sessions` the session captures a frozen binding — the profile's model roles and launch/capability policy, its provider→pool execution target (strategy/retry/affinity/memberships), and the provider config — alongside the pinned model universe. Profile-route resolution (`DEFAULT`, `EXACT_CLIENT_MODEL`, tier/alias, `INHERIT`) reads the FROZEN binding overlaid on the live policy, so a mid-session control-plane edit, rename, or removal of the profile/pool/provider can never silently change an active session's model or execution target (and never surfaces a `role-unmapped` for a selector the session was launched with). Account state, credentials, and ECR compatibility stay LIVE — revocations, pauses, and quarantines apply immediately. Config changes take effect for NEW sessions. `EffectiveModelDecision` records both the live `policyRevision`/`policyHash` and the pinned `sessionUniverseRevision`, so an observer can still see the base policy revision each request resolved.
...
### Effective Compatibility Registry (#124) — sole runtime compatibility authority

The Effective Compatibility Registry (`src/compatibility/`) is the SOLE runtime compatibility authority. It converts observations into explicit reviewed trust decisions and exposes ONE effective answer per exact claim/feature for the runtime consumers (selector #68, tier resolver #69, projection #72, reasoning/tool eligibility, `resolve-route`). The model registry KEEPS owning model identity/capability evidence and is never duplicated; legacy static `model.compatibility.state` values are seed/reference data only.

- **Review Decision Store** (`ReviewDecisionStore`, `<control-plane>/compat/reviews/`): explicit `promote`/`reject` decisions tied to the exact #122 claim identity AND evidence revision (deterministic digest of the claim's observation history via `evidenceRevisionFor`). A new PASS observation never auto-promotes; a decision no longer covering the current evidence revision marks the claim review-stale (`untrusted`, re-review required). Append/audit-friendly monotonic revisions, fail-closed schema-validated reads, `0700`/`0600`.
- **Negative Quarantine Store** (`QuarantineStore`, `<control-plane>/compat/quarantines/`): a strong reproducible failure quarantines an EXACT claim/path/feature (narrow scope by keying — one provider/model/feature failure never poisons unrelated paths); quarantine never deletes historical evidence; explicit lift is audit-friendly. Required features fail closed (`quarantined-fail-closed`).
- **Freshness/staleness engine** (`freshness.ts`): evidence is marked STALE on client version/baseline drift, protocol/adapter revision drift, provider endpoint/auth-mode drift, physical model fingerprint change, fixture/corpus revision drift, material RLY build change, and evidence age — a stale positive never remains silently VERIFIED.
- **Effective resolution** (`effective.ts`): evidence + review + quarantine + freshness + policy resolve into `trusted`/`stale`/`experimental`/`untrusted`/`quarantined`/`missing` with trust, observed health, freshness, quarantine, and enforcement reason kept as DISTINCT diagnosable fields — never a single persisted boolean. Deterministic precedence: active quarantine → missing evidence → failed evidence → explicit rejection → decision no longer covering current evidence → promote + stale → promote + fresh + passed → unreviewed PASS (`experimental` at most).
- **Enforcement** (`enforceEffective`): required features fail closed unless effectively trusted; the explicit experimental override (exact-pin opt-in or `allowExperimental`) is traceable and visible in route traces/doctor and can elevate `experimental`/`stale` but NEVER `untrusted`/`missing`/`quarantined`; a hard quarantine is bypassable only through the separately documented administrative `allowQuarantineBypass` policy (visible as `quarantine-bypass`).
- **Registry facade** (`registry.ts`): `EffectiveCompatibilityRegistry` ties the claim/review/quarantine stores + pinned runtime policy together, derives exact claim identities from registry rows under the pinned client baseline/protocol revision, resolves per-feature answers, builds in-memory snapshots for the pure routing consumers, and exposes secret-free `explain`/`summary` for doctor/status.
- **Runtime wiring**: `owned-gateway` constructs the ECR from the control-plane directory + the pinned client contract + the material RLY build (`RUNTIME_VERSION`); the gateway server threads it into `resolveProfileRoute`/`resolveProjectedModelRoute` and the `GET /v1/models` route; the selector/tier/projection consume the snapshot and recompute enforcement per request context. `rly compat status|review promote|reject|quarantine|lift|explain` is the operator surface; doctor reports `effectiveCompatibility` counts + pinned policy.
- **Migration/audit**: `seed-reference-only` policy — a static `VERIFIED` row without a `claimRef` derives `experimental` at best and can never silently satisfy reviewed v2 trust; `BROKEN` stays a hard negative seed. Auditability is preserved on trust change (monotonic revisions, append-only records) and evidence expiry (freshness reasons).
