...
- **Bootstrap installer** (`scripts/install.sh`): fail-closed POSIX sh for clean per-user macOS/Linux installs — no source/npm/pnpm/user Node, no sudo. Verifies the artifact digest chain in-shell (sha256 vs published `.sha256` AND both signed metadata documents; Ed25519 digest statement via OpenSSL 3 `pkeyutl -rawin` against the committed public key; unavailable openssl fails closed), then runs `rly install --artifact <tarball> --metadata-dir <staging>` which re-verifies the FULL canonical-JSON signature chain before any install mutation.
