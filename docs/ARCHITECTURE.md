...
- **Documented live-config contract for profile routes**: session pinning applies to the PROJECTION namespace only. A request whose selector resolves through the profile route (`DEFAULT`, `EXACT_CLIENT_MODEL`, tier/alias, `INHERIT` without a recorded parent) reads the CURRENT control-plane policy at request time by design — a mid-session profile edit takes effect on the next request (or surfaces a typed `role-unmapped` when the selector is no longer mapped). This divergence is never silent: `EffectiveModelDecision` records BOTH the live `policyRevision`/`policyHash` and the pinned `sessionUniverseRevision`, so an observer can see exactly which revision resolved each request. This is the documented contract; a future "freeze the whole session at launch" option (contract A) would be a separate explicit product decision.
