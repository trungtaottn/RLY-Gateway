...
async function temporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), "rly-gateway-cline-claude-e2e-"));
...
async function seedAndConfig(directory: string, endpoint: string): Promise<{
  config: string;
  homeFile: string;
  clineSource: string;
  ...
  let seededPaths: { clineSource: string; codexCredential: string };
  try {
    const sentinel = await seedCodexCredentialFile(broker, controlPlane);
    const seeded = await seedClineClaudeProfile(store, broker, controlPlane, { endpoint, profileName: "clinepass" });
    seededPaths = {
      clineSource: join(controlPlane, seeded.accounts[0]?.pseudonym ?? "acct-cline-a", "cline-auth.json"),
      codexCredential: sentinel.activePath,
    };
  } finally {
    await broker.close();
    store.close();
  ...
  await writeFile(join(directory, ".codex", "config.toml"), "sentinel-codex\n", "utf8");
  return { config, homeFile, clineSource: seededPaths.clineSource, codexCredential: seededPaths.codexCredential };
}

describe.skipIf(!enabled)("Claude Code fake Cline interop E2E", () => {
  it("runs text through rly clinepass without mutating Claude, Cline, or Codex files", async () => {
    ...
    provider.post("/chat/completions", (request) => {
      ...
      return new Response(sseFixture("cline-e2e", "FAKE_CLINE_CLAUDE_E2E_OK"), {
    ...
    const beforeCline = await digest(seeded.clineSource);
    const beforeCodex = await digest(seeded.codexCredential);
    const result = await run(process.execPath, [
      "dist/cli/main.js", "clinepass", "--config", seeded.config, "--",
      "-p", "Reply with the fixture marker.", "--dangerously-skip-permissions",
    ], { ...process.env, HOME: directory }).completion;
    expect(result.code, result.output).toBe(0);
    expect(result.output).toContain("FAKE_CLINE_CLAUDE_E2E_OK");
    expect(received.length).toBeGreaterThan(0);
    expect(received.every((model) => model === CLINE_PRIMARY_MODEL)).toBe(true);
    expect(await digest(seeded.homeFile)).toBe(beforeClaude);
    expect(await digest(seeded.clineSource)).toBe(beforeCline);
    expect(await digest(seeded.codexCredential)).toBe(beforeCodex);
  }, timeoutMs);

  it("completes a tool round-trip through cline-interop", async () => {
    ...
    provider.post("/chat/completions", (request) => {
      ...
      const response = toolResultReceived
        ? sseFixture("cline-tool-final", "TOOL_ROUNDTRIP_OK")
        : [
          'data: {"id":"cline-tool-call","choices":[{"delta":{"tool_calls":[{"index":0,"id":"call_bash","function":{"name":"Bash","arguments":"{\\"command\\":\\"printf fixture-tool\\"}"}}]}}]}\n\n',
          'data: {"id":"cline-tool-call","choices":[{"delta":{},"finish_reason":"tool_calls"}],"usage":{"prompt_tokens":2,"completion_tokens":1}}\n\n',
          "data: [DONE]\n\n",
        ].join("");
  ...
  it("does not mutate Codex credential files when Cline upstream fails", async () => {
    provider = Fastify();
