...
import { CODEX_PROFILE_ROLES } from "../../helpers/codex-profile-seed.js";
...
describe.skipIf(!enabled || !handle || !claude.found)("Claude Code live Codex OAuth smoke", () => {
  it("issues one Claude Code request through a Codex profile without mutating global config", async () => {
    const directory = await mkdtemp(join(tmpdir(), "rly-gateway-codex-claude-live-"));
    directories.push(directory);
    const sourceDirectory = process.env["RLY_LIVE_CODEX_CONTROL_PLANE"] ?? defaultControlPlaneDirectory();
    ...
    try {
      ...
      store.createProfile({
        ...
        providerId: provider.id,
        poolId: pool.id,
        modelRoles: CODEX_PROFILE_ROLES,
      }, "cli");
    } finally {
      store.close();
    }
    ...
    const result = await run(process.execPath, [
      "dist/cli/main.js", "codex", "--config", config, "--",
