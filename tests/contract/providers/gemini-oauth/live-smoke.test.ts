...
const enabled = process.env["AGENT_GATEWAY_LIVE_GEMINI_OAUTH"] === "1";
...
describe.skipIf(!enabled)("Gemini OAuth live smoke", () => {
  it("requires a project-owned client id and never impersonates Gemini CLI or Code Assist", () => {
    expect(process.env[GEMINI_CLIENT_ID_ENV], "set AGENT_GATEWAY_GEMINI_OAUTH_CLIENT_ID for live smoke").toBeTruthy();
