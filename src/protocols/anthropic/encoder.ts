...
export function aggregateAnthropicEvents(events: readonly CanonicalEvent[]): Record<string, unknown> {
  ...
  const blocks = new Map<number, Record<string, unknown>>(); const toolJson = new Map<number, string>(); let responseId = "msg_unknown"; let model = "unknown"; let stopReason: string | null = null; let usage: Record<string, number> = {};
  for (const item of events) {
